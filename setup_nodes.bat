...
if exist "%BASE_DIR%\%%N@DESKTOPQ2A2FL7_data" (
    ...
    start powershell -NoExit -Command "[console]::ForegroundColor = 'White'; [console]::BackgroundColor = 'DarkBlue'; Clear-Host;& erl -sname %%N@DESKTOPQ2A2FL7 -setcookie %COOKIE% -pa %BASE_DIR% %BASE_DIR%\ebin -pa %BASE_DIR%\src -config %BASE_DIR%\config\%%N"
)
REM Avvia il nodo di monitor service 
 start powershell -NoExit -Command "[console]::ForegroundColor = 'White'; [console]::BackgroundColor = 'DarkGreen'; Clear-Host;&erl -sname Monitor_service@DESKTOPQ2A2FL7 -setcookie %COOKIE% -pa %BASE_DIR%  -pa %BASE_DIR%\ebin  -pa %BASE_DIR%\src -eval "cluster_setup:start_cluster`(`)" -eval "observer:start`(`)" -eval "global:registered_names`(`)"
